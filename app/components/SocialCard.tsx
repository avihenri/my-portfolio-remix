...
const SocialCard: React.FC<SocialCardProps> = ({ username, desc, service, href, icon }) => {
  return (
      <div className="overflow-hidden relative duration-300 border rounded-xl dark:bg-zinc-200 dark:hover:bg-zinc-300 hover:bg-zinc-800 group bg-black hover:border-zinc-400 border-zinc-600 w-72 md:w-80 max-w-full h-auto sm:h-96 transition-transform hover:scale-105">
        ...
        <div className="z-10 flex flex-col items-center">
          <span className="text-base sm:text-lg font-medium duration-150 xl:text-3xl dark:text-black text-zinc-200 dark:group-hover:text-slate-800 group-hover:text-white font-display">{username}</span>
          <span className="mt-1 text-xs sm:mt-4 sm:text-sm text-center duration-1000 text-zinc-400 dark:text-black dark:group-hover:text-slate-800 group-hover:text-zinc-200">{desc}</span>
